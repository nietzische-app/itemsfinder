 ...
 *
 * A scan is Vision (≤3.5s) followed by the optional Context.dev stage, which
 * is bounded by `CONTEXT_DEV_DEADLINE_MS` (4s by default). 60s leaves headroom
 * above that; the live stage's deadline must always stay below this number, or
 * the function is killed mid-flight and the client gets a platform error
 * instead of the catalogue fallback the deadline exists to trigger.
 *
